...
import (
	...
	"github.com/gocircuit/escher/github.com/onsi/gomega/gexec"
	"io/ioutil"
	"os"
	"path/filepath"
	"strings"
...
var _ = Describe("Subcommand", func() {
	Describe("ginkgo bootstrap", func() {
		It("should generate a bootstrap file, as long as one does not exist", func() {
			...
			Ω(content).Should(ContainSubstring("\t" + `. "github.com/gocircuit/escher/github.com/onsi/ginkgo"`))
			Ω(content).Should(ContainSubstring("\t" + `. "github.com/gocircuit/escher/github.com/onsi/gomega"`))
	...
	Describe("ginkgo generate", func() {
		...
		Context("with nodot", func() {
			It("should not import ginkgo or gomega", func() {
				...
				content, err := ioutil.ReadFile(filepath.Join(pkgPath, "foo_bar_test.go"))
				Ω(err).ShouldNot(HaveOccurred())
				Ω(content).ShouldNot(ContainSubstring("\t" + `. "github.com/gocircuit/escher/github.com/onsi/ginkgo"`))
				Ω(content).ShouldNot(ContainSubstring("\t" + `. "github.com/gocircuit/escher/github.com/onsi/gomega"`))
			})
		})
	})
