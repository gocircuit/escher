...
func addGinkgoImports(rootNode *ast.File) {
	...
	for _, importSpec := range importDecl.Specs {
		...
		if importSpec.Path.Value == "\"github.com/gocircuit/escher/kit/github.com/onsi/ginkgo\"" {
			needsGinkgo = false
		}
	}

	if needsGinkgo {
		importDecl.Specs = append(importDecl.Specs, createImport(".", "\"github.com/gocircuit/escher/kit/github.com/onsi/ginkgo\""))
	}
}
